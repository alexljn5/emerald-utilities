...
### Added
- Tailscale bridge for safe development access to <server-hostname>-Server PostgreSQL
...
### Changed
- `config.json` no longer hardcodes the public IP `<public-ip>`;
  safe `127.0.0.1` default now
- `src/.env` and `src/database/.env.example` now use MagicDNS
  `DB_HOST=<server-hostname>` (Tailscale-bridged)
- `OLLAMA_HOST` now points at `http://<server-hostname>:11434`
