...
export async function getTasks() {
    ...
    await ensureTasksSchema(p);
    const result = await p.query(
        `SELECT id, title, description, completed, archived, priority, due_time, reminder_time, long_term, created_at, updated_at
         FROM tasks
...
export async function createTask(task) {
    ...
    await ensureTasksSchema(p);
    const result = await p.query(
        `INSERT INTO tasks (title, description, priority, due_time, reminder_time, long_term)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id, title, description, completed, archived, priority, due_time, reminder_time, long_term, created_at, updated_at`,
        [
            task.title,
