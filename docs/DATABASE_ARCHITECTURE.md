...
Emerald Utilities uses PostgreSQL 16 with pgvector for AI embeddings, running in a Docker container on the homelab server (`<server-hostname>-Server`).
...
Emerald Utilities
        ...
        |
        v
PostgreSQL (Docker on <server-hostname>-Server)
        ↑
...
|----------|---------|---------|
| `DB_HOST` | Database host | `<server-hostname>` (MagicDNS) |
| `DB_PORT` | Database port | `5432` |
| `DB_NAME` | Database name | `emerald_utilities` |
| `DB_USER` | Database user | `emerald-user` |
