...
export function buildConversationContext({
    ...
    for (const msg of recent || []) {
        ...
        const entry = {
            role: msg.author === 'example-user' ? 'user' : msg.author === 'Example Assistant' ? 'assistant' : msg.author,
            content: msg.content || '',
...
/**
 * Default example character sheet.
 * Used as fallback when no character sheet is provided.
 *
 * Generic placeholder content for local setup.
 */
...
export function buildSystemPrompt(character, additionalInstructions = '') {
    ...
    // This preserves locally configured personality text.
