...
0 2 * * * cd /home/alexljn5/emerald-utilities/src/database && ./scripts/db-backup.sh >> /var/log/emerald-db-backup.log 2>&1
```
...
docker compose exec postgres psql -U alexljn5 -d postgres -c "CREATE DATABASE emerald_test;"

# Restore backup to test database
docker compose exec postgres pg_restore -U alexljn5 -d emerald_test /path/to/backup.dump

# Verify
docker compose exec postgres psql -U alexljn5 -d emerald_test -c "SELECT COUNT(*) FROM grok_messages;"

# Clean up
docker compose exec postgres psql -U alexljn5 -d postgres -c "DROP DATABASE emerald_test;"
```
