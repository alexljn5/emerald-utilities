...
3. Restore to a temporary database first to verify:
   ...
   docker compose exec postgres psql -U emerald-user -d postgres -c "CREATE DATABASE emerald_verify;"

   # Restore to temp
   docker compose exec postgres pg_restore -U emerald-user -d emerald_verify /path/to/backup.dump

   # Verify data
   docker compose exec postgres psql -U emerald-user -d emerald_verify -c "SELECT COUNT(*) FROM grok_messages;"

   # If good, restore to production
   ./scripts/db-restore.sh /path/to/backup.dump

   # Clean up temp
   docker compose exec postgres psql -U emerald-user -d postgres -c "DROP DATABASE emerald_verify;"
   ```
