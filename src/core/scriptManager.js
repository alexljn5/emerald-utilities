...
export class ScriptManager {
    ...
    renderScripts() {
        ...
        this.renderFn(
            Array.isArray(this.state.config._files)
                ? this.state.config._files
                : [],
    ...
    normalizeScripts() {
        const scripts = this.state.config?.scripts;
        if (!scripts) return [];
    ...
    async tryAutoRun() {
        ...
        const scripts = this.normalizeScripts();

        for (const file of this.state.config._files || []) {
            const cfg = scripts.find((script) => script.file === file);
            if (!cfg) continue;
