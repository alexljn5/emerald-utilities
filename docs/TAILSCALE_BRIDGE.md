...
PostgreSQL/pgvector database on `<server-hostname>` using Tailscale.
...
`<server-hostname>` Tailscale address:
...
The Tailscale installation and authentication on `<server-hostname>` are
already complete.
...
Emerald should not care whether `<server-hostname>` is reachable through
...
`<public-ip>` as a hardcoded default. This is now a safe localhost
...
```bash
# 1. Confirm Tailscale sees <server-hostname>
tailscale status
...
psql "host=<server-hostname> port=5432 dbname=emerald_utilities user=emerald-user"
```
...
Emerald → Tailscale → Remote PostgreSQL
...
**Windows (PowerShell):**
```bash
ssh-keygen -t ed25519 -C "emerald-user@<server-hostname>" -f $env:USERPROFILE\.ssh\id_ed25519 -N ""
```

**Linux / macOS (Bash):**
```bash
ssh-keygen -t ed25519 -C "emerald-user@<server-hostname>" -f ~/.ssh/id_ed25519 -N ""
```
...
**Windows (PowerShell):**
```bash
cat $env:USERPROFILE\.ssh\id_ed25519.pub | ssh -o StrictHostKeyChecking=no emerald-user@<server-hostname> "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys"
```

**Linux / macOS (Bash):**
```bash
cat ~/.ssh/id_ed25519.pub | ssh -o StrictHostKeyChecking=no emerald-user@<server-hostname> "/bin/mkdir -p ~/.ssh && /bin/cat >> ~/.ssh/authorized_keys"
```
...
**Windows (PowerShell):**
```bash
ssh -o StrictHostKeyChecking=no -o BatchMode=yes -i $env:USERPROFILE\.ssh\id_ed25519 emerald-user@<server-hostname> "echo ok"
```

**Linux / macOS (Bash):**
```bash
ssh -o StrictHostKeyChecking=no -o BatchMode=yes -i ~/.ssh/id_ed25519 emerald-user@<server-hostname> "echo ok"
```
...
```json
{
    ...
    "sshUser": "emerald-user",
    "sshPort": "22",
    "sshKey": "C:\\Users\\emerald\\.ssh\\id_ed25519",
    "autoStart": true
}
```
