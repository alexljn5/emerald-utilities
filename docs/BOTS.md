...
`<server-hostname>` is reachable through Tailscale, LAN, localhost, or another network is
...
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o BatchMode=yes \
    -p 22 -i C:\\Users\\emerald\\.ssh\id_ed25519 emerald-user@<server-hostname> "docker ps -a"
```

The `<server-hostname>` hostname is resolved by the OS — Tailscale's MagicDNS handles the
...
|----------|---------|---------|
| `BOT_SSH_HOST` | SSH host (Tailscale MagicDNS or IP) | `<server-hostname>` (from `bot-config.json`) |
| `BOT_SSH_USER` | SSH username | `emerald-user` |
| `BOT_SSH_PORT` | SSH port | `22` |
| `BOT_SSH_KEY` | Path to SSH private key | `~/.ssh/id_ed25519` |
| `BOT_TAILSCALE_HOST` | Override Tailscale MagicDNS hostname | `<server-hostname>` |
...
ssh -o StrictHostKeyChecking=no -o BatchMode=yes \
    -i ~/.ssh/id_ed25519 emerald-user@<server-hostname> "echo ok"
...
```bash
# Start the bot
screen -dmS infbot bash -c "cd /home/emerald-user/<server-hostname>/infbot && node src/bot-entry.js; exec bash"
...
1. Copy the bot source to your homelab:
   ```bash
   scp -r src/containers/infbot-src/ user@homelab-host:/home/emerald-user/<server-hostname>/infbot/
   ```
