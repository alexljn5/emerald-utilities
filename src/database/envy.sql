...
-- Requires the pgvector/pgvector image (or the extension installed manually).
...
-- RAG embedding column (768-dim, matches nomic-embed-text via Ollama).
-- Added conditionally so this is safe on both fresh and existing databases;
-- it NEVER drops an existing embedding column (that would destroy vectors).
ALTER TABLE grok_messages ADD COLUMN IF NOT EXISTS embedding vector(768);
...
-- RAG vector index. MUST use vector_cosine_ops because rag-query.js ranks with
-- the cosine-distance operator (<=>). An L2 (vector_l2_ops) index cannot serve
-- a `<=>` ORDER BY and the planner silently falls back to a full sequential
-- scan. `lists = 16` is a sane starting point for ~10k-30k rows; rebuild with a
-- larger value (roughly rows/1000) as the dataset grows. Building on an empty
-- column is cheap; rag-setup.sh rebuilds it with a data-driven `lists` value.
